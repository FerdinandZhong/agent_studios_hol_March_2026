...
## Design Note: Why RAG for Intelligence Screening (Task 4)?

Tasks 2 and 5 use SQL (`iceberg-mcp-server`) against structured trade databases. Task 4 uses RAG (`rag_studio_tool`) against an unstructured Intelligence Knowledge Base. The choice is intentional and distinct for each data type.

### Why RAG is the right tool for intelligence data

**1. The data is unstructured narrative, not rows.**
Fraud bulletins, sanctions records, and intelligence reports are documents — they contain narrative context, case descriptions, and relational detail that does not map cleanly to SQL columns. RAG treats them as searchable text and preserves the full content.

**2. OCR-extracted entity names require semantic matching, not exact matching.**
Names extracted from scanned invoices carry spelling variants, abbreviations, and transliteration differences (`Textile House Ltd` vs `Textile Hse Ltd` vs `TextileHouse Limited`). A SQL `WHERE` clause fails silently on these variations. RAG vector embeddings cluster semantically similar strings, so near-identical names retrieve the correct record regardless of minor OCR errors.

**3. The task requires LLM reasoning beyond a simple lookup.**
After retrieval, the LLM assesses whether a near-match is a genuine alias, a subsidiary, or a coincidence. SQL can return a binary match; it cannot reason about organizational structure, naming conventions, or transliteration patterns.

### Where this design has a gap

For **formal sanctions lists** (OFAC SDN, EU Consolidated List, UN Security Council), RAG is a weaker choice. Sanctions compliance requires:
- Deterministic, auditable matching — exact record, listing date, legal basis
- Structured metadata: jurisdiction, regulation, listing authority
- A defined match threshold with a documented audit trail

A probabilistic similarity score is not sufficient for enforcement action on formal sanctions grounds.

### Recommended tool split for production

| Intelligence type | Recommended tool |
|---|---|
| Fraud bulletins, seizure records, intelligence narratives | RAG — semantic search + LLM reasoning |
| Formal sanctions lists (OFAC, EU, UN) | SQL or dedicated sanctions screening API |
| Known fraud network and corridor patterns | RAG — narrative and relational context |
| Watchlist exact name matching | SQL with fuzzy matching (Levenshtein / soundex) |

> **Implementation note:** The current workflow collapses both intelligence types into a single RAG KB. For a production deployment where findings will be cited in enforcement action, formal sanctions screening should be separated into a dedicated SQL query against a regularly-updated structured list. RAG is then reserved for the unstructured intelligence layer where its semantic search and LLM reasoning capabilities are uniquely valuable.
